create function public.get_public_leaderboard(p_range text, p_month date)
returns table (
  rank bigint,
  display_name text,
  visit_count bigint,
...
with bounds as (
  select
    case
      when p_range = 'month' then date_trunc('month', p_month::timestamp)
      when p_range = 'year' then date_trunc('year', now() at time zone 'Europe/Stockholm')
      when p_range = 'total' then null::timestamp
      else null::timestamp
    end as start_local,
    case
      when p_range = 'month' then date_trunc('month', p_month::timestamp) + interval '1 month'
      else null::timestamp
    end as end_local
),
filtered_attendance as (
  ...
  cross join bounds b
  where p_range = 'total'
     or (p_range = 'year' and (a.incheckad at time zone 'Europe/Stockholm') >= b.start_local)
     or (p_range = 'month'
         and (a.incheckad at time zone 'Europe/Stockholm') >= b.start_local
         and (a.incheckad at time zone 'Europe/Stockholm') < b.end_local)
),
visit_counts as (
...
revoke all on function public.get_public_leaderboard(text, date) from public;
grant execute on function public.get_public_leaderboard(text, date) to anon, authenticated;
